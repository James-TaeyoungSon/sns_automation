# -*- coding: utf-8 -*-
"""웹 기사 스크래핑 — auto_mysns_posting/article_reader.py 에서 복사."""
import re
...
def fetch_article(url: str, timeout: int = 20) -> Article:
    ...
    title = _first_meta(soup, ["og:title", "twitter:title"]) or _text_or_empty(soup.find("title"))
    if not title:
        h1 = soup.find("h1")
        title = _text_or_empty(h1)

    description = _first_meta(soup, ["og:description", "twitter:description", "description"])
    site_name = _first_meta(soup, ["og:site_name"]) or urlparse(url).netloc
    ...
    return Article(
        url=url,
        title=_clean_space(title) or url,
...
def _remove_noise(soup: BeautifulSoup) -> None:
    for tag in soup(["script", "style", "noscript", "svg", "form", "iframe", "nav", "footer", "aside", "button"]):
        tag.decompose()
...
def _extract_main_text(soup: BeautifulSoup) -> str:
    candidates = []
    for selector in ["#newsct_article", "#dic_area", "#articeBody", "article", "main", "[role='main']", ".article", ".post", ".content"]:
